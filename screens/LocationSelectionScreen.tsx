...
export default function LocationSelectionScreen(
  ...
  const onPressOutOfTown = (location: ILocationOption) => {
    ...
    if (location.isBSDDuties) {
      props.navigation.navigate(TYPE_A_DUTIES_STACK_SCREEN_NAME, {
        location: 'BSD',
      });
    } else {
        ...
        <ScrollView style={styles.scrollView}>
          {isTownState ? (
            <View style={styles.scrollBox}>
              {IN_TOWN_LOCATIONS.map(
                (location: ILocationOption, index: number) => {
                  return (
                    <View style={styles.colBox} key={index}>
                      <PanelButton
                        label={location.name}
