...
export default function RouteContainer(props: IRouteContainer) {
  ...
  const RenderDrawerContainer = () => {
    return <NavDrawer.Navigator
      initialRouteName={props.config.initialRouteName}
      drawerContent={(drawerProps) => <NavigationDrawer {...drawerProps} config={props.config} />}
    >
      {
        props.config.screens.filter(s => (s.isScreen)).map((screen: IRouteScreen, index: number) => {
          return <NavDrawer.Screen
            key={index}
            name={screen.name}
