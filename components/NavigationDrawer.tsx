...
import { ICustomDrawerContentComponentProps, IRouteScreen } from '../types/UIType';
...
export default function NavigationDrawer(props: ICustomDrawerContentComponentProps) {
  ...
  const handleMenu = (screen: IRouteScreen): void => {
    Logger.log(MethodFormat(`${handleMenu.name}`, `screen name: ${screen.name}`), `${NavigationDrawer.name}`);
    props.navigation.navigate(screen.name);
  }

  const handleLogout = (): void => {
    Logger.log(MethodFormat(`${handleLogout.name}`, ''), `${NavigationDrawer.name}`);
    ...
    <View style={styles.main}>
      ...
      <View style={styles.body}>
        <DrawerContentScrollView>
          {
            props.config.screens.filter((screen) => !screen.isHide).map((screen: IRouteScreen, index: number) => {
              return <MenuButton
                key={index}
